...
class adsSetup {
    ...
    public function _content_install_settings() {
        ...
        $shoppage = get_page_by_title( $this->_get_page_name_locale( 'Shop' ) );
        if ( $shoppage ) {
            update_option( 'woocommerce_shop_page_id',$shoppage->ID );
        }
        $shoppage = get_page_by_title( $this->_get_page_name_locale('Cart' ) );
        if ( $shoppage ) {
            update_option( 'woocommerce_cart_page_id',$shoppage->ID );
        }
        $shoppage = get_page_by_title( $this->_get_page_name_locale( 'Checkout' ) );
        if ( $shoppage ) {
            update_option( 'woocommerce_checkout_page_id',$shoppage->ID );
        }
        $shoppage = get_page_by_title( $this->_get_page_name_locale( 'My Account' ) );
        if ( $shoppage ) {
            update_option( 'woocommerce_myaccount_page_id',$shoppage->ID );
        }
        $shoppage = get_page_by_title( $this->_get_page_name_locale( 'Terms & Conditions' ) );
        if ( $shoppage ) {
            update_option( 'woocommerce_terms_page_id', $shoppage->ID );
        }
        $homepage = get_page_by_title( $this->_get_page_name_locale( 'Home' ) );
        if ( $homepage ) {
            update_option( 'page_on_front', $homepage->ID );
            update_option( 'show_on_front', 'page' );
        }
        $blogpage = get_page_by_title( $this->_get_page_name_locale('Blog' ) );
        if ( $blogpage ) {
